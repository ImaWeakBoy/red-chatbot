...
class Olami {
  constructor(
    appKey = config.olami.appKey,
    appSecret = config.olami.appSectet,
    inputType = 1
  ) {
      ...
      .post(
        ...
        {},
        {
          params: {
            ...
            timestamp: timestamp,
            sign: md5(
              `${this.appSecret}api=nliappkey=${
                this.appKey
              }timestamp=${timestamp}${this.appSecret}`
            ),
            cusid: cusid,
            rq: JSON.stringify({
              data_type: 'stt',
              data: { input_type: this.inputType, text: text }
            })
          }
        }
      )
  ...
  _intentDetection(nli) {
        ...
        case 'poem':
          return new TextMessage(
            desc.result +
              '\n\n' +
              data
                .map((el, index) => index + 1 + '. ' + el.poem_name)
                .join('\n')
          );
        case 'cooking':
          return new TextMessage(
    ...
    async function handleMusicKKBOXType(semantic) {
      ...
      const data = await api.searchFetcher
    ...
    switch (type) {
      case 'kkbox':
        return data.length > 0
          ? new TextMessage(data[0].url)
          : new TextMessage(desc.result);
      case 'baike':
